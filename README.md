...
- **Opening an article re-downloads it from the source**, for every feed, whatever *Parse full content* is set to — the saved copy (or the feed's own text) is shown straight away and swapped for the fresh one when it arrives; if the download fails, what is on screen stays.
...
- **Unread count on the app icon** (*Settings → Interaction*, off by default). Android has **no API for this**: since Oreo a launcher badge is drawn purely from an app's active notifications, and the number on it comes from `setNumber()`. So the only way to keep a count on the icon is to keep a notification alive carrying it — this posts one silent, ongoing, low-importance notification that tracks the unread count and disappears when it reaches zero. Whether the *number* is drawn is up to your launcher: **Samsung One UI draws it**, stock Pixel shows a plain dot.
- **The reader remembers where you stopped** in each of the last 300 articles you read, and reopens them there. The position is restored only once the page is tall enough to hold it: the WebView grows as its images load, and scrolling past the end of a document that is still short silently does nothing.
- **Last synced 5 minutes ago** under the title on both the feed list and the article list, in your local time zone. It re-ticks while the page is open. If the last sync failed it says **Sync failed — pull to retry** instead, rather than leaving the last *successful* time on screen as if the list were up to date.
...
- **Emoji and tracking pixels became thumbnails.** The list thumbnail was simply the first `<img>` in the article. WordPress rewrites emoji into `<img>` tags from `s.w.org`, so a post opening with an emoji got a 72×72 ⭐ as its picture; spacer GIFs and 1×1 trackers did the same. The first *plausible* content image now wins.
