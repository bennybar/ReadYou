...
- **Predictive back.** Dragging back now peels the current screen away under your finger and reveals the one behind it, so you can see where you are going and abandon the gesture if it is not where you wanted. Upstream had the animation built — `NavDisplay` already declared a `predictivePopTransitionSpec` — but the manifest set `android:enableOnBackInvokedCallback="false"`, which kept the whole app on the legacy back path so it never once fired. The predictive animation is also its own thing now rather than a copy of the ordinary pop: a pop only has to get out of the way (it slid a tenth of the width), whereas a gesture you are scrubbing has to actually show you the destination.
